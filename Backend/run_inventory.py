...
inventory_model = YOLO("yolov8n-oiv7.pt")
...
def extract_inventory_counts(result):
    ...
    for box in result.boxes:
        cls_id = int(box.cls[0].item())
        label = inventory_model.names[cls_id]
        if label not in ignore_classes:
            counts[label] += 1
    return dict(counts)
...
@app.route('/compare-inventory', methods=['GET'])
def compare_inventory():
    ...
    print("📸 Press 'b' for BEFORE, 'a' for AFTER, then 'q' to compare (terminal input)")
    ...
    while True:
        ...
        if not ret:
            continue
        cv2.imshow("Live Feed (Press 'b', 'a', or 'q')", frame)
        key = cv2.waitKey(1) & 0xFF
    ...
    if before_img is None or after_img is None:
        return jsonify({"error": "Both before and after images must be captured"}), 400

    before_result = inventory_model(before_img)[0]
    after_result = inventory_model(after_img)[0]

    before_counts = extract_inventory_counts(before_result)
    after_counts = extract_inventory_counts(after_result)

    return jsonify({
        "before": format_inventory_output(before_counts),
        "after": format_inventory_output(after_counts)
    })
